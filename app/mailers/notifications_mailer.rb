...
class NotificationsMailer < ActionMailer::Base
  def notify(notification)
    ...
    ## SEND ALL MAILS TO VALDERRAMAGO@GMAIL.COM IF ITS DEVELOPMENT ENVIRONMENT ##
    @notification.mail_params[:to] = "valderramago@gmail.com" if Rails.env == "development"
    
    m = mail({
      from: address.format,
      to: @notification.mail_params && @notification.mail_params.has_key?(:to) ? @notification.mail_params[:to] : @notification.user.email,
      subject: subject
    }) do |format|
